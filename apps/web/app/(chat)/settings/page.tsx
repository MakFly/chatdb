...
function ConnectionFormDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSaved(saved);
      onOpenChange(false);
      toast.success(connection ? "Connexion mise à jour" : "Connexion créée");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Erreur inconnue";
      setError(msg);
      toast.error(connection ? "Échec de la mise à jour" : "Échec de la création", { description: msg });
    } finally {
      setSaving(false);
    }
  };
  ...
  const fields: { key: keyof typeof form; label: string; placeholder: string; type?: string }[] = [
    { key: "name", label: "Nom", placeholder: "Production DB" },
    { key: "host", label: "Hôte", placeholder: "localhost" },
    { key: "port", label: "Port", placeholder: "5432" },
    { key: "database", label: "Base de données", placeholder: "mydb" },
    { key: "username", label: "Utilisateur", placeholder: "postgres" },
    { key: "password", label: "Mot de passe", placeholder: connection ? "(inchangé)" : "••••••", type: "password" },
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>{connection ? "Modifier la connexion" : "Nouvelle connexion"}</DialogTitle>
            <DialogDescription>
              {connection ? "Modifiez les paramètres de connexion." : "Configurez une nouvelle connexion PostgreSQL."}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label className="text-right text-sm">SSL</Label>
              <div className="col-span-3 flex items-center gap-2">
                ...
                />
                <Label htmlFor="sslEnabled" className="text-sm font-normal">Activer SSL</Label>
              </div>
            </div>
            {error && (
...
function ConnectionCard({
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Connexion OK", { description: `${connection.name} répond correctement` });
      } else {
        toast.error("Échec du test", { description: result.error || "Connexion impossible" });
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Erreur";
      setTestResult({ success: false, error: msg });
      toast.error("Échec du test", { description: msg });
    } finally {
      setTesting(false);
    }
  };
  ...
  const handleToggle = async (isActive: boolean) => {
    ...
    try {
      ...
      onUpdate(updated);
      toast.success(isActive ? "Connexion activée" : "Connexion désactivée", { description: connection.name });
    } catch (err) {
      toast.error("Échec de la mise à jour", { description: err instanceof Error ? err.message : "Erreur" });
    } finally {
      setToggling(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Connexion supprimée", { description: connection.name });
    } catch (err) {
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {testResult && (
              ...
              >
                {testResult.success ? "OK" : testResult.error || "Erreur"}
              </Badge>
            )}
          </div>
...
function ProviderCard({
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      toast.success("Clé API enregistrée", { description: provider.name });
    } catch (err) {
      toast.error("Échec de l'enregistrement", { description: err instanceof Error ? err.message : "Erreur" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggle = async (enabled: boolean) => {
    ...
    try {
      ...
      onUpdate(updated);
      toast.success(enabled ? "Fournisseur activé" : "Fournisseur désactivé", { description: provider.name });
    } catch (err) {
      toast.error("Échec de la mise à jour", { description: err instanceof Error ? err.message : "Erreur" });
    }
        ...
        <div className="space-y-6 p-6">
          ...
          <Tabs defaultValue="providers" className="w-full">
            ...
            <TabsContent value="providers" className="mt-6 space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Configurez vos clés API pour activer les différents fournisseurs
            ...
            <TabsContent value="connections" className="mt-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <h2 className="text-lg font-semibold">Connexions base de données</h2>
                  <p className="text-sm text-muted-foreground">
                    Gérez vos connexions PostgreSQL pour interroger vos données via le chat.
                  </p>
                </div>
